...
class Ec2Inventory(object):
    ...
    def read_settings(self):
        ...
        if config.has_option("ec2", "destination_format") and config.has_option(
            "ec2", "destination_format_tags"
        ):
        ...
        self.route53_excluded_zones = [
            a for a in config.get("ec2", "route53_excluded_zones").split(",") if a
        ]
        ...
        self.boto_profile = (
            ...
            or config.get("ec2", "boto_profile")
        )
        ...
        if (
            config.has_option("ec2", "instance_filters")
            or "EC2_INSTANCE_FILTERS" in os.environ
        ):
            filters = os.getenv(
    ...
    def get_rds_instances_by_region(self, region):
        ...
        try:
            ...
            if conn:
                ...
                while True:
                    ...
                    for index, instance in enumerate(instances):
                        ...
                        tags = client.list_tags_for_resource(ResourceName=instance.arn)[
                            "TagList"
                        ]
                        instance.tags = {}
                        for tag in tags:
            ...
            if e.error_code == "AuthFailure":
                ...
                error = (
                    "RDS hasn't been enabled for this account yet. "
    ...
    def get_elasticache_clusters_by_region(self, region):
        ...
        try:
            ...
            if e.error_code == "AuthFailure":
                ...
                error = (
                    "ElastiCache hasn't been enabled for this account yet. "
    ...
    def add_instance(self, instance, region):
        ...
        # to_safe strips hostname characters like dots, so don't strip route53 hostnames
            ...
            self.route53_enabled
            and self.route53_hostnames
            and hostname.endswith(self.route53_hostnames)
        ):
    ...
    def add_rds_instance(self, instance, region):
        ...
        # Inventory: Group by VPC
        if (
            self.group_by_vpc_id
            and instance.subnet_group
            and instance.subnet_group.vpc_id
        ):
    ...
    def add_elasticache_cluster(self, cluster, region):
        ...
        if (
            not self.all_elasticache_clusters
            and cluster["CacheClusterStatus"] != "available"
        ):
            return
        ...
        if self.group_by_availability_zone and not is_redis:
            ...
            if self.nested_groups:
                if self.group_by_region:
                    self.push_group(
                        self.inventory, region, cluster["PreferredAvailabilityZone"]
                    )
                self.push_group(
                    self.inventory, "zones", cluster["PreferredAvailabilityZone"]
                )
        ...
        if self.group_by_elasticache_engine and not is_redis:
            self.push(
                self.inventory, self.to_safe("elasticache_" + cluster["Engine"]), dest
            )
            if self.nested_groups:
                self.push_group(
                    self.inventory,
        ...
        if (
            self.group_by_elasticache_replication_group
            and "ReplicationGroupId" in cluster
            and cluster["ReplicationGroupId"]
        ):
            self.push(
                self.inventory,
                self.to_safe(
                    "elasticache_replication_group_" + cluster["ReplicationGroupId"]
                ),
                dest,
            )
            if self.nested_groups:
    ...
    def add_elasticache_replication_group(self, replication_group, region):
        ...
        if (
            not self.all_elasticache_replication_groups
            and replication_group["Status"] != "available"
        ):
            return

        # Skip clusters we cannot address (e.g. private VPC subnet or clustered redis)
        if (
            replication_group["NodeGroups"][0]["PrimaryEndpoint"] is None
            or replication_group["NodeGroups"][0]["PrimaryEndpoint"]["Address"] is None
        ):
            return
        ...
        if self.group_by_instance_id:
            ...
            if self.nested_groups:
                self.push_group(
                    self.inventory, "instances", replication_group["ReplicationGroupId"]
                )
    ...
    def get_host_info_dict_from_describe_dict(self, describe_dict):
        ...
        for key in describe_dict:
            ...
            if key == "ec2_node_groups" and value:
                host_info["ec2_endpoint_address"] = value[0]["PrimaryEndpoint"][
                    "Address"
                ]
                ...
                for node in value[0]["NodeGroupMembers"]:
                    if node["CurrentRole"] == "primary":
                        host_info["ec2_primary_cluster_address"] = node["ReadEndpoint"][
                            "Address"
                        ]
                        host_info["ec2_primary_cluster_port"] = node["ReadEndpoint"][
                            "Port"
                        ]
                        host_info["ec2_primary_cluster_id"] = node["CacheClusterId"]
                    elif node["CurrentRole"] == "replica":
                        host_info[
                            "ec2_replica_cluster_address_" + str(replica_count)
                        ] = node["ReadEndpoint"]["Address"]
                        host_info[
                            "ec2_replica_cluster_port_" + str(replica_count)
                        ] = node["ReadEndpoint"]["Port"]
                        host_info[
                            "ec2_replica_cluster_id_" + str(replica_count)
                        ] = node["CacheClusterId"]
                        replica_count += 1
