...
data "template_cloudinit_config" "config" {
  ...
  part {
    ...
    content      = templatefile("${path.module}/init.sh", { user = var.user, github_user = var.github_user, host_name = var.host_name, full_name = var.full_name, email = var.email, env = var.env, dns = var.dns_name })
